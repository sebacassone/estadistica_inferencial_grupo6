...
Como no siguen una distribución normal y los datos son muestras Pareadas, se usara la prueba no parametrica "Prueba de rangos con signo de Wilcoxon", ya que no se puede usar la prueba t studenen ni la z.

Las condicions para la Prueba de rangos con signo de Wilcoxon son :
 - 1. Los pares de observaciones son independientes
 - 2. La escala de  medicón empleada para las observaiones es intrínsecamente continua
 - 3. La escala de medicón empleada para ambas muestras debe ser a lo menos ordinal

La condición 1 se cumple porque cada instancia no depende de la otra, la condición 2 se cumple ya que la escala de medición es el tiempo y puede tomar cualquier valor dentro de un intervalo especificado como lo son los milisegundos. La condición 3 se cumple ya que los datos son cuantitativos y continuos

Ho: No hay diferencia en el tiempo de los algoritmos en la misma insntancia con respecto la media de los tiempos de ejecución entre las versiones A y C del algoritmo para instancias con 60 o más nodos.
...
prueba1 <- wilcoxsign_test(muestras_A$tiempo.A,muestras_C$tiempo.C, alternative = "two.sided", paired = TRUE, conf.level = 1-alfa)
